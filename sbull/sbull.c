...
//#include <linux/genhd.h> // is removed at linux kernel 6.1.2
...
enum {
	...
	RM_FULL = 1, // The full_blown version
...
static int sbull_open(struct block_device *bdev, fmode_t mode)
{
	...
	printk(KERN_WARNING "sbull: sbull_open function called\n");
	dump_stack();
				...
				unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd) {
		...
		case BLKGETSIZE: // added
			size = dev->size * hardsect_size / KERNEL_SECTOR_SIZE;
			return put_user(size, (long *) arg);

		case BLKSSZGET: // added
			return put_user(dev->size * hardsect_size, (int *) arg);
/*
		case BLKFLSBUF:
			if(!capable(CAP_SYS_ADMIN))
				return -EACCES;
			destroy_buffers(bdev->bd_inode->i_rdev, cmd, arg);
*/
		default:
			return -EINVAL;
	}
...
static void setup_device(struct sbull_dev *dev, int which)
{
	...
	switch(request_mode) {
...
#endif
			if(dev->queue == NULL) {
				printk(KERN_WARNING "sbull: RM_NOQUEUE, dev->queue is NULL\n");
				goto out_vfree;
			}
			break;
		...
		case RM_FULL:
			...
			if (err) {
				printk(KERN_WARNING "sbull: RM_FULL, error on blk_mq_alloc_tag_set function\n");
				goto out_vfree;
			}
...
//				goto out_vfree;
		...
		case RM_SIMPLE:
			...
			dev->tag_set.nr_hw_queues = 1; // added
			dev->tag_set.queue_depth = 128;
			dev->tag_set.numa_node = NUMA_NO_NODE; // added
			dev->tag_set.cmd_size = 0; // added
			dev->tag_set.flags = BLK_MQ_F_SHOULD_MERGE;
			err = blk_mq_alloc_tag_set(&dev->tag_set);

			if (err) {
				dump_stack(); 
				printk(KERN_WARNING "sbull: RM_SIMPLE, err on blk_mq_alloc_tag_set function\n");
				goto out_vfree;
			}
...
out_free_tag_set:
	...
	printk(KERN_WARNING "sbull: out_free_tag_set called\n");
out_vfree:
	if(dev->data)
		vfree(dev->data);
	printk(KERN_WARNING "sbull: out_vfree called\n");
}
...
static int __init sbull_init(void)
{
	...
	for(i = 0; i < ndevices; i++) {
		printk(KERN_WARNING "sbull: device registration %d\n", (i+1));
		setup_device(Devices + i, i);
	}

	printk(KERN_WARNING "sbull: device registration done\n");

	return 0;

out_unregister:
	unregister_blkdev(sbull_major, "sbd");
	printk(KERN_WARNING "sbull: out_unregister called\n");
	return -ENOMEM;
}
...
static void sbull_exit(void)
{
	...
	printk(KERN_WARNING "sbull: device exit function done\n"); 
}
